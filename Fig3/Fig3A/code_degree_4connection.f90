        ...
        implicit none
        integer :: i, mi, ri, j, k, time, maxtime, numruns, found, neighno
        integer :: bp, bnp, shift, bonds, step, chk, chk2, flip_pass
        integer :: cntNP, cntP, cnt,cnt1(10000), cnt2, bond_pass, cntt1, cntt2, flip_cnt
        integer :: cnt_bond, cnt_nobond, started, nodes, node_degree(10000), n_degree(1000)
        integer :: cntx1, cntx2, cnty1, cnty2, cnt1_total, cntxx1, cntxx2, cntyy1, cntyy2, cnt2_total
        real(8), dimension(10000) :: bx, by, bxx, byy, f, ff, xposnext, yposnext, xg, yg
        real(8), dimension(10000) :: bpx, bpy, bnpx, bnpy, flagp, flagnp, xpos, ypos, final_degree
        real(8) :: x, y, t, Delta_t, x_rec, y_rec, tau, sumdegree, zero_degree
 ...
 close(121)
!***************************************************       
     open(189,file ="grid_pos_10X10.dat")
   do i = 1, nodes
   read(189,*) xg(i), yg(i)
   enddo
!************************************************** 
...
!****************************movement starts**********************************************************
          if (t .gt. 10.0*tau .and. started .eq. 0) then
           ...
           do i = 1, bonds             
            if (f(i) .eq. 1) then
             write(555,*)    xpos(i), ypos(i)
             write(555,*)   xposnext(i), yposnext(i)
             write(555,*)
             write(555,*)
             endif
            enddo
            ...
            do k = 1 , nodes   
              ...
              node_degree(k) = 0
               ...
               do i = 1, bonds
                 ...
                 if ((f(i) .eq. 1) .and. (bx(i) .lt. xg(k)) .and. (bx(i) .gt. (xg(k) - 1.0)) .and. (by(i) .eq. yg(k))) then
                      node_degree(k) = node_degree(k) + 1
                 endif        
             

                 if ((f(i) .eq. 1) .and.(bx(i) .gt. xg(k)) .and. (bx(i) .lt. (xg(k) + 1.0)) .and. (by(i) .eq. yg(k))) then
                     node_degree(k) = node_degree(k) + 1
                 endif  

                
                 if ((f(i) .eq. 1) .and. (by(i) .gt. yg(k)) .and. (by(i) .lt. (yg(k) + 1.0)) .and. (bx(i) .eq. xg(k))) then 
                     node_degree(k) = node_degree(k) + 1
                 endif       
            

                 if ((f(i) .eq. 1) .and.(by(i) .lt. yg(k)) .and. (by(i) .gt. (yg(k) - 1.0)) .and. (bx(i) .eq. xg(k))) then      
                    node_degree(k) = node_degree(k) + 1
                 endif 
             ...
             do i = 1, nodes
               n_degree(node_degree(i)) = n_degree(node_degree(i)) + 1
             enddo
            

             sumdegree = 0.0
              do i = 1, 4
             final_degree(i) = n_degree(i)! / real(nodes)
             sumdegree = sumdegree + n_degree(i)
             enddo


             open(127,file = "node_info.dat")
             zero_degree = (nodes - sumdegree)
             write(127,*) 0, zero_degree !/ real(nodes)
           
             do i = 1, 4
             write(127,*) i, final_degree(i)
             enddo
             close(127)
          endif
...
!***********************************************************************************************************
        ...
        if ((cnt1_total .lt. neighno) .and. (cnt2_total .lt. neighno)) then
          f(i) = 1
        endif   
